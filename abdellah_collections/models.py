...
class Collection(models.Model):
    ...
    def max_price(self):
        ...
        from products.models import Product
        max_price = Product.objects.filter(collection=self, is_available=True).aggregate(Max('price'))['price__max']
        return max_price
    
    @property
    def total_products(self):
        """Return the total number of available products in this collection"""
        from products.models import Product
        return Product.objects.filter(collection=self, is_available=True).count()
    
    @property
    def my_products(self):
        """
        Returns a list of the first three product names associated with this collection.
        """
        from products.models import Product
        return list(
            Product.objects.filter(collection=self, is_available=True)
            .order_by('id')[:2]
